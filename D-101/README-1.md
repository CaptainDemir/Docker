...
Bookstore Web API Application aims to create a bookstore web service using Docker to give Devops Team the understanding to dockerization of an application. The application code is to be deployed as a RESTful web service with Flask using Dockerfile and Docker Compose on AWS Elastic Compute Cloud (EC2) Instance using Terraform.

## Problem Statement

![Project_D-103](D-103-bookstore-api.png) 
...
|----bookstore-api.py   # Given to the Devops Team (Python Flask Web API)
|----requirements.txt   # Given to the Devops Team(List of Flask modules for Web API)
|----main.tf            # To be delivered by Devops Team (Terraform config file)
|----docker-compose.yml # To be delivered by Devops Team
|----Dockerfile         # To be delivered by Devops Team
```
